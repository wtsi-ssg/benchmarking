...
RABBITMQ_IP=`docker inspect s3-rabbit | jq --raw-output '.[].NetworkSettings.Networks.bridge.IPAddress'`
docker run -d --hostname s3-consume --name s3-consume -e AWS_ACCESS_KEY_ID=$ACCESS_KEY -e AWS_SECRET_ACCESS_KEY=$SECRET_ACCESS_KEY wsisci/benchmarking_s3processing $RABBITMQ_IP $RABBIT_USER $RABBIT_PASSWORD "$DSN"