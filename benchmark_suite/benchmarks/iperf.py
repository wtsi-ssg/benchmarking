...
class IPerf(benchmarkessentials.Benchmark):
    ...
    def run(self):
        ...
        print(f"--Version: {self.programversion}, Protocol: {self.protocol}")
        ...
        with subprocess.Popen(
            ...
            if stdout_dict["end"]:
                if self.protocol == "TCP":
                    run_results["sum_sent"] = (
                        stdout_dict["end"]["sum_sent"]
                        if stdout_dict["end"]["sum_sent"]
                        else {}
                    )
                    run_results["sum_received"] = (
                        stdout_dict["end"]["sum_received"]
                        if stdout_dict["end"]["sum_received"]
                        else {}
                    )
                elif self.protocol == "UDP":
                    run_results["sum"] = (
                        stdout_dict["end"]["sum"] if stdout_dict["end"]["sum"] else {}
                    )

                run_results["cpu_utilization_percent"] = (
                    stdout_dict["end"]["cpu_utilization_percent"]
                    if stdout_dict["end"]["cpu_utilization_percent"]
                    else {}
                )
            else:
