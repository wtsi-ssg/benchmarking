...
class PlotResults:
    ...
    def __init__(
        ...
        compare_filenames: list,
        plot_filename: pathlib.Path,
        cost_per_kwh: decimal.Decimal,
        carbon_per_kwh: decimal.Decimal,
        override_power: decimal.Decimal,
        override_tco: decimal.Decimal,
    ) -> None:
    ...
    def get_result_cpu_single_data(
        main_results, data
    ) -> tuple[list, list, list, list, list, list]:
        return [
            list(
                itertools.chain.from_iterable(
                    itertools.repeat(m["threads"], len(m["runs"]))
            ...
            list(
                sum(float(x["value"]) for x in m["power"].values())
                for keyData in data["results"]["configurations"]
                for m in keyData["runs"]
                if keyData["processes"] == 1
            ),
    ...
    def find_matching_reports(report, compare_results):
        for compare_result in compare_results:
            if report in compare_result["results"]["CPU"]["benchmarks"]:
                yield compare_result["nickname"], compare_result["results"]["CPU"][
                    "benchmarks"
                ][report]

    def get_result_cpu_throughput_data(
        main_results, data
    ) -> "list[list,list,list,list,list,list,list]":
        return [
            list(
                itertools.chain.from_iterable(
                    itertools.repeat(m["processes"], len(m["runs"]))
                    for m in data["results"]["configurations"]
                    if m["processes"] * m["threads"]
                    == main_results["system-info"]["cpuinfo"]["count"]
                )
            ),  # processes
            list(
                itertools.chain.from_iterable(
                    itertools.repeat(f"{m['processes']}*{m['threads']}", len(m["runs"]))
                    for m in data["results"]["configurations"]
                    if m["processes"] * m["threads"]
                    == main_results["system-info"]["cpuinfo"]["count"]
                )
            ...
            list(
                ...
                for keyData in data["results"]["configurations"]
                for m in keyData["runs"]
                if keyData["processes"] * keyData["threads"]
                == main_results["system-info"]["cpuinfo"]["count"]
            ),
            list(
                m["system"]
    ...
    def plot_CPU_single_process(
        self, main_results: dict, compare_results: "list[dict]", pdf: PdfPages
    ):
        for report, data in main_results["results"]["CPU"]["benchmarks"].items():
            ...
            for subdata in data:
                ...
                (
                    ...
                    x_rss,
                    x_power_per_run,
                ) = PlotResults.get_result_cpu_single_data(
                    main_results["system-info"]["model"], subdata
                )
                if len(x_labels) == 0:
                    continue
                models_list = [np.tile(main_results["nickname"], len(x_labels))]
                # bring in results from matching tests in comparison reports
                for model, matching_result in PlotResults.find_matching_reports(
                    main_results, compare_results
                ):
                    ...
                    (
                        ...
                        main_results, matching_result[0]
                    )
                ...
                ax.set_ylabel(
                    "User-mode + System CPU time (Seconds)", fontweight="semibold"
                )
        ...
        for report, data in main_results["results"]["CPU"]["benchmarks"].items():
            ...
            for subdata in data:
                ...
                (
                    ...
                    x_rss,
                    x_power_per_run,
                ) = PlotResults.get_result_cpu_throughput_data(main_results, subdata)
                ...
                for model, matching_result in PlotResults.find_matching_reports(
                    ...
                    (
                        ...
                        m_rss,
                        m_power_per_run,
                    ) = PlotResults.get_result_cpu_throughput_data(
                        main_results, matching_result[0]
                    )
                ...
                x_outputs = [
                    process / (elapsed / 3600)
                    for process, elapsed in zip(x_processes, x_elapsed)
                ]
                ...
                rects3 = ax.bar(
                    ind, x_elapsed_mean, width, yerr=x_elapsed_std, label="Elapsed"
                )
                ...
                ind = np.arange(
                    len(x_power_per_run_mean)
                )  # the x locations for the groups
                width = 0.20  # the width of the bars

                rects3 = ax.bar(
                    ind,
                    x_power_per_run_mean,
                    width,
                    yerr=x_power_per_run_std,
                    label="Power per Output (kWh)",
                )
                # TODO: make this right hand axis if defined
                if self.carbon_per_kwh:
                    # carbon per run
                    ax2 = ax.secondary_yaxis(
                        "right", functions=(self.kwh_to_co2, self.co2_to_kwh)
                    )
    ...
    def plot_MBW(main_results: dict, compare_results: "list[dict]", pdf: PdfPages):
        for report, data in main_results["results"]["CPU"]["benchmarks"].items():
            ...
            for model, matching_result in PlotResults.find_matching_reports(
                ...
                m_user = list(
                    float(x["copy"].split(" ")[0])
                    for x in matching_result[0]["results"]
                    if x["method"] == "MEMCPY"
                )
    ...
    def plot_disk(results: dict, pdf: PdfPages):
        for report, data in results["results"]["Disk"]["benchmarks"]["IOZone"][0][
            "results"
        ].items():
            x = pd.DataFrame(data)
    ...
    def plot_iperf(results: dict, pdf: PdfPages):
        x_labels = ("UDP",)
        x = (
            results["results"]["Network"]["benchmarks"]["iPerf"][0]["result_summary"][
                "sum"
            ]["bits_per_second"],
        )
    ...
    def plot_title(results: dict, pdf: PdfPages):
        ...
        plt.text(
            0.5,
            0.5,
            f"Benchmarking results for {results['system-info']['model']}\n{results['date']}",
            ha="center",
            va="center",
        )
        pdf.savefig(fig)
    ...
    def plot_results(self):
        ...
        # Begin plot
        with PdfPages(
            str(self.plot_filename),
            metadata={
                "Creator": "Genomics Benchmark",
                "Title": f"Benchmarking results for {self.results['system-info']['model']}\n{self.results['date']}",
            },
        ) as pdf:
